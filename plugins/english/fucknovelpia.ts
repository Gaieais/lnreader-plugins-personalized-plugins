...
class FuckNovelpia implements Plugin.PluginBase {
  ...
  async parseChapter(chapterPath: string): Promise<string> {
    ...
    // Keep the reader's semantic HTML (headings, paragraphs, inline elements,
    // lists, quotes, tables, and media) instead of reducing the chapter to text.
    chapter
      .find(
        '.reader-nav, #reader-bottom, .reader-controls, #reader-controls, .install-banner',
      )
      .remove();
    chapter.find('script, style, noscript, template').remove();

    let chapterUrl = this.site;
    try {
      chapterUrl = new URL(chapterPath, this.site).href;
    } catch {
      // Keep the site root as a base when a malformed path is supplied.
    }
    ...
    chapter.find('*').each((_, el) => {
      ...
      for (const attribute of ['href', 'src', 'poster']) {
        ...
        try {
          $el.attr(attribute, new URL(value, chapterUrl).href);
        } catch {
          // Leave an unparseable URL unchanged rather than dropping content.
        }
      }
    });
    ...
    return (
      chapter
        ...
        // Some imported EPUB chapters contain escaped HTML line breaks.
        .replace(/&lt;br\s*\/?&gt;/gi, '<br />') ?? ''
    );
  }
